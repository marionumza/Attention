...
class KsDashboardNinjaBoard(models.Model):
    ...
    def ks_fetch_dashboard_data(self, ks_dashboard_id, ks_item_domain=False):
        ...
        dashboard_data = {
            ...
            'ks_date_filter_selection': self._context.get('ksDateFilterSelection',
                                                          self.browse(ks_dashboard_id).ks_date_filter_selection),
        ...
        for data in ks_dashboard_data:
            ...
            if data['ks_item_data']:
                ...
                for item in data['ks_item_data']:
                    ...
                    ks_model_id = item['ks_model_id']
                    item['ks_model_id'] = self.env['ir.model'].search([('model', '=', item['ks_model_id'])]).id
                    ...
                    if ks_action_lines and len(ks_action_lines) != 0:
                        ...
                        for line in ks_action_lines:
                            if line['ks_item_action_field']:
                                ...
                                for x in self.env['ir.model'].search([('model', '=', ks_model_id)]).modules.split(
                                        ", "):
                                    record_id = x + '.field_' + ks_model + "__" + ks_item_action_field
                                    ks_record_id = self.env.ref(record_id, False)
                                    if ks_record_id:
