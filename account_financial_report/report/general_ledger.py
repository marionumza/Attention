...
class GeneralLedgerReportCompute(models.TransientModel):
    ...
    @api.multi
    def _calc_initial_retain_general(self, report_id):
        mytrial = self.env['report_general_ledger_account'].sudo().search(
            [('code', '>=', '4000000'), ('code', '<=', '9999902'), ('report_id', '=', report_id)])


        # print(myretain_earning.code, myretain_earning.final_balance)
        # for x in myretain_earning:
        #     print(x.code, x.final_balance)

        for rec in mytrial:
          print(report_id)
          print(rec.code )

          MYamount = self.env['report_general_ledger_move_line'].sudo().search(
              [('account', '=',rec.code  ),('report_account_id', '=', rec.id)])
          print(MYamount)
          total_final_debit = 0
          total_final_credit = 0
          for myrec in MYamount:
              total_final_debit +=myrec.debit
              total_final_credit +=myrec.credit

          rec.sudo().write({'initial_debit': 0, 'initial_credit': 0
...
"""
        ...
        query_inject_account_params = {
            ...
            'final_credit':  initial_debit +unaffected_earnings_period_credit,
            'final_balance': final_balance,
        }
        print(initial_debit + unaffected_earnings_period_debit)
        print( unaffected_earnings_period_debit)
        # haytham
        # 'final_debit': initial_debit + unaffected_earnings_period_debit,
