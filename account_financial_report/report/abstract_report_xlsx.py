...
class AbstractReportXslx(models.AbstractModel):
    ...
    def write_array_header(self, date_at=False):
        ...
        for col_pos, column in self.columns.items():
            ...
            if date_at:
                date = date_at.strftime('%B')
                col = col_pos
                if date == column['header']:
                    break
        if self._name == 'report.a_f_r.report_aged_partner_balance_xlsx' and self.columns.get(18):
            self.sheet.write(self.row_pos, col+1, self.columns.get(18).get('header'),
                             self.format_header_center)
            self.row_pos += 1
    ...
    def write_line(self, line_object):
        ...
        """
        balance = 0.0
        if self._name == 'report.a_f_r.report_aged_partner_balance_xlsx':
            final_year = datetime.datetime.now().year - 1
            final_date = datetime.datetime.now().replace(day=31, month=12, year=final_year)
            account_move_id = self.env['account.move'].search([('date', '=', final_date), ('journal_id.code', '=', 'OPEJ'), ('journal_id.type', '=', 'situation')], limit=1)
            if account_move_id:
                account_move_partner_lines = account_move_id.line_ids.filtered(lambda r:r.partner_id.id == line_object.report_partner_id.partner_id.id)
        ...
        for col_pos, column in self.columns.items():
            ...
            date = False
            if self._name == 'report.a_f_r.report_aged_partner_balance_xlsx':
                date = line_object.report_partner_id.report_account_id.report_id.date_at.strftime('%B')
            ...
            elif column['field'] == 'payment_terms':
                self.sheet.write_string(self.row_pos, col_pos, line_object.report_partner_id.partner_id.property_payment_term_id.name or '')
            elif column['field'] == 'sales_person':
                self.sheet.write_string(self.row_pos, col_pos, line_object.report_partner_id.partner_id.user_id.name or '')
            ...
            if date and date == column['header']:
                self.sheet.write_number(
                    self.row_pos, col_pos + 1, float(balance), cell_format
                )
                break
        self.row_pos += 1
