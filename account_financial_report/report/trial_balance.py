...
class TrialBalanceReportCompute(models.TransientModel):
    ...
    @api.multi
    def _calc_initial_retain(self,report_id):
        mytrial = self.env['report_trial_balance_account'].search(
            [('code', '>=', '4000000'), ('code', '<=', '9999902'),('report_id', '=', report_id)])

        myretain_earning = self.env['report_trial_balance_account'].search(
            [('code', '=', '3000003'),('report_id', '=', report_id)])
        ...
        for rec in mytrial:
            if rec.initial_balance !=0:
                Total +=rec.initial_balance
                rec.final_balance = rec.final_balance - rec.initial_balance
                # print(rec.code ,rec.initial_balance)
                rec.initial_balance=0

        myretain_earning.final_balance +=Total
        myretain_earning.intial_balance =  myretain_earning.final_balance
