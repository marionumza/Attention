...
class AgedPartnerBalanceReportAccount(models.TransientModel):
    ...
    cumul_total = fields.Float(digits=(16, 2))
    ...
    cumul_age_150_days = fields.Float(digits=(16, 2))
    cumul_age_180_days = fields.Float(digits=(16, 2))
    cumul_age_210_days = fields.Float(digits=(16, 2))
    cumul_age_240_days = fields.Float(digits=(16, 2))
    cumul_age_270_days = fields.Float(digits=(16, 2))
    cumul_age_300_days = fields.Float(digits=(16, 2))
    cumul_age_330_days = fields.Float(digits=(16, 2))
    cumul_age_360_days = fields.Float(digits=(16, 2))

    cumul_older = fields.Float(digits=(16, 2))

    percent_current = fields.Float(digits=(16, 2))
    percent_total = fields.Float(digits=(16, 2))
    ...
    percent_age_150_days = fields.Float(digits=(16, 2))
    percent_age_180_days = fields.Float(digits=(16, 2))
    percent_age_210_days = fields.Float(digits=(16, 2))
    percent_age_240_days = fields.Float(digits=(16, 2))
    percent_age_270_days = fields.Float(digits=(16, 2))
    percent_age_300_days = fields.Float(digits=(16, 2))
    percent_age_330_days = fields.Float(digits=(16, 2))
    percent_age_360_days = fields.Float(digits=(16, 2))
...
class AgedPartnerBalanceReportPartner(models.TransientModel):
    ...
    property_payment_term_id = fields.Many2one('account.payment.term', default=_compute_property_payment_term_id)
...
class AgedPartnerBalanceReportLine(models.TransientModel):
    ...
    age_150_days = fields.Float(digits=(16, 2))
    age_180_days = fields.Float(digits=(16, 2))
    age_210_days = fields.Float(digits=(16, 2))
    age_240_days = fields.Float(digits=(16, 2))
    age_270_days = fields.Float(digits=(16, 2))
    age_300_days = fields.Float(digits=(16, 2))
    age_330_days = fields.Float(digits=(16, 2))
    age_360_days = fields.Float(digits=(16, 2))
    ...
    date_range AS
        (
            SELECT
                ...
                DATE %s - INTEGER '150' AS date_less_150_days,
                DATE %s - INTEGER '180' AS date_less_180_days,
                DATE %s - INTEGER '210' AS date_less_210_days,
                DATE %s - INTEGER '240' AS date_less_240_days,
                DATE %s - INTEGER '270' AS date_less_270_days,
                DATE %s - INTEGER '300' AS date_less_300_days,
                DATE %s - INTEGER '330' AS date_less_330_days,
                DATE %s - INTEGER '360' AS date_less_360_days
        )
INSERT INTO
...
SELECT
    ...
    (SUM(
        CASE
            ...
            THEN rlo.amount_residual
        END
    )+SUM(rlo.amount_residual)) AS total ,
    ...
    SUM(
        CASE
            WHEN
                rlo.date_due >= date_range.date_less_150_days
    ...
    SUM(
        CASE
            WHEN
                rlo.date_due >= date_range.date_less_180_days
                AND rlo.date_due < date_range.date_less_150_days
            THEN rlo.amount_residual
        END
    ) AS age_180_days,
    SUM(
        CASE
            WHEN
                rlo.date_due >= date_range.date_less_210_days
                AND rlo.date_due < date_range.date_less_180_days
            THEN rlo.amount_residual
        END
    ) AS age_210_days,
    SUM(
        CASE
            WHEN
                rlo.date_due >= date_range.date_less_240_days
                AND rlo.date_due < date_range.date_less_210_days
            THEN rlo.amount_residual
        END
    ) AS age_240_days,
    SUM(
        CASE
            WHEN
                rlo.date_due >= date_range.date_less_270_days
                AND rlo.date_due < date_range.date_less_240_days
            THEN rlo.amount_residual
        END
    ) AS age_270_days,
    SUM(
        CASE
            WHEN
                rlo.date_due >= date_range.date_less_300_days
                AND rlo.date_due < date_range.date_less_270_days
            THEN rlo.amount_residual
        END
    ) AS age_300_days,
    SUM(
        CASE
            WHEN
                rlo.date_due >= date_range.date_less_330_days
                AND rlo.date_due < date_range.date_less_300_days
            THEN rlo.amount_residual
        END
    ) AS age_330_days,
    SUM(
        CASE
            WHEN
                rlo.date_due >= date_range.date_less_360_days
                AND rlo.date_due < date_range.date_less_300_days
            THEN rlo.amount_residual
        END
    ) AS age_360_days,
    SUM(
        CASE
            WHEN rlo.date_due < date_range.date_less_360_days
            THEN rlo.amount_residual
        END
    ) AS older
FROM
...
GROUP BY
    rp.id
        """
        query_inject_line_params = (self.date_at,) * 13
...
SELECT
    ...
    CASE
        WHEN rlo.date_due >= date_range.date_current
        THEN rlo.amount_residual
    END + rlo.amount_residual as total,
    CASE
        WHEN
            rlo.date_due >= date_range.date_less_30_days
            AND rlo.date_due < date_range.date_current
    ...
    CASE
        WHEN
            rlo.date_due >= date_range.date_less_360_days
            AND rlo.date_due < date_range.date_less_330_days
        THEN rlo.amount_residual
    END AS age_360_days,
    CASE
        WHEN rlo.date_due < date_range.date_less_360_days
...
AND ra.report_id = %s
        """
        query_inject_move_line_params = (self.date_at,) * 13
    ...
    cumuls AS
        (
            SELECT
                ...
                SUM(rl.age_150_days) AS cumul_age_150_days,
                SUM(rl.age_180_days) AS cumul_age_180_days,
                SUM(rl.age_210_days) AS cumul_age_210_days,
                SUM(rl.age_240_days) AS cumul_age_240_days,
                SUM(rl.age_270_days) AS cumul_age_270_days,
                SUM(rl.age_300_days) AS cumul_age_300_days,
                SUM(rl.age_330_days) AS cumul_age_330_days,
                SUM(rl.age_360_days) AS cumul_age_360_days,
...
SET
    ...
    cumul_total = (c.cumul_current + c.cumul_amount_residual),
    ...
    cumul_age_150_days = c.cumul_age_150_days,
    cumul_age_180_days = c.cumul_age_180_days,
    cumul_age_210_days = c.cumul_age_210_days,
    cumul_age_240_days = c.cumul_age_240_days,
    cumul_age_270_days = c.cumul_age_270_days,
    cumul_age_300_days = c.cumul_age_300_days,
    cumul_age_330_days = c.cumul_age_330_days,
    cumul_age_360_days = c.cumul_age_360_days,
    ...
    percent_age_120_days =
        ...
        END,
    percent_age_150_days =
        CASE
            WHEN c.cumul_amount_residual != 0
            THEN 100 * c.cumul_age_150_days / c.cumul_amount_residual
        END,
    percent_age_180_days =
        CASE
            WHEN c.cumul_amount_residual != 0
            THEN 100 * c.cumul_age_180_days / c.cumul_amount_residual
        END,
    percent_age_210_days =
        CASE
            WHEN c.cumul_amount_residual != 0
            THEN 100 * c.cumul_age_210_days / c.cumul_amount_residual
        END,
    percent_age_240_days =
        CASE
            WHEN c.cumul_amount_residual != 0
            THEN 100 * c.cumul_age_240_days / c.cumul_amount_residual
        END,
    percent_age_270_days =
        CASE
            WHEN c.cumul_amount_residual != 0
            THEN 100 * c.cumul_age_270_days / c.cumul_amount_residual
        END,
    percent_age_300_days =
        CASE
            WHEN c.cumul_amount_residual != 0
            THEN 100 * c.cumul_age_300_days / c.cumul_amount_residual
        END,
    percent_age_330_days =
        CASE
            WHEN c.cumul_amount_residual != 0
            THEN 100 * c.cumul_age_330_days / c.cumul_amount_residual
        END,
    percent_age_360_days =
        CASE
            WHEN c.cumul_amount_residual != 0
            THEN 100 * c.cumul_age_360_days / c.cumul_amount_residual
        END,
    percent_older =
        CASE
            WHEN c.cumul_amount_residual != 0
