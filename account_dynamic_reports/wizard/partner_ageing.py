...
class InsPartnerAgeing(models.TransientModel):
    ...
    bucket_1 = fields.Integer(string='Bucket 1', required=True, default=lambda self:self.env['res.company'].search([('id','=',1)]).bucket_1)
    bucket_2 = fields.Integer(string='Bucket 2', required=True, default=lambda self:self.env['res.company'].search([('id','=',1)]).bucket_2)
    bucket_3 = fields.Integer(string='Bucket 3', required=True, default=lambda self:self.env['res.company'].search([('id','=',1)]).bucket_3)
    bucket_4 = fields.Integer(string='Bucket 4', required=True, default=lambda self:self.env['res.company'].search([('id','=',1)]).bucket_4)
    bucket_5 = fields.Integer(string='Bucket 5', required=True, default=lambda self:self.env['res.company'].search([('id','=',1)]).bucket_5)
    ...
    company_id = fields.Many2one(
        ...
        default=lambda self: self.env['res.company'].search([('id','=',1)])
    )
    ...
    def process_data(self):
        ...
        domain = ['|',('company_id','=',self.env['res.company'].search([('id','=',1)]).id),('company_id','=',False)]
        if self.partner_type == 'customer':
